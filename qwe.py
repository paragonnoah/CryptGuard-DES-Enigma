...
def str_to_bin(user_input):
    # Decode bytes to string if input is bytes
    if isinstance(user_input, bytes):
        user_input = user_input.decode('utf-8')

    # Convert integer to string if input is integer
...
def encryption(input_file, output_file, key):
    ...
    #print("Binary representation of input:", binary_rep_of_input)  # Debugging statement
...
def decryption(input_file, output_file, key):
    ...
    # Check if the binary representation is 64 bits long
    if len(ip_dec_result_str) != 64:
        raise ValueError("Binary representation must be 64 bits long.")
